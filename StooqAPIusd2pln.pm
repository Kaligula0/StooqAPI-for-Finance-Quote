...
#
package Finance::Quote::StooqAPIusd2pln;
require 5.004;
...
sub methods { return (stooq_api_usd2pln => \&stooq_api_usd2pln); }

{
  my @labels = qw/date isodate time method source name currency last price open high low ask bid volume/;
	
  sub labels { return (stooq_api_usd2pln => \@labels); }
}

sub stooq_api_usd2pln {
  my $quoter  = shift;
  my @symbols = @_;
  ...
  foreach $symbol (@symbols) {
    ...
    unless ($date eq "N/A") {
		...
		$stocks{$symbol, 'method'}   = 'stooq_api_usd2pln';
		$stocks{$symbol, 'source'}   = 'Finance::Quote::StooqAPIusd2pln';
...
=head1 SYNOPSIS
    ...
    %stockinfo = $q->fetch("stooq_api_usd2pln","tlt"); # the letter ticker
