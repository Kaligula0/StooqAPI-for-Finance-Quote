...
#
package Finance::Quote::StooqAPIgbx;
require 5.004;
...
sub stooq_api_gbx {
  ...
  foreach my $symbol (@symbols) {
    ...
    unless ($date eq "N/A") {
		...
		$stocks{$symbol, 'method'}   = 'stooq_api_gbx';
		$stocks{$symbol, 'source'}   = 'Finance::Quote::StooqAPIgbx';

		$stocks{$symbol, 'currency'} = 'GBP';
	
		# divide GBX (GBp) quotes by 100
...
=head1 SYNOPSIS
    ...
    %stockinfo = $q->fetch("stooq_api_gbx","tlt"); # the letter ticker
