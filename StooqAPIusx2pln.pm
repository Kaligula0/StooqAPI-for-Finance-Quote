...
#
package Finance::Quote::StooqAPIusx2pln;
require 5.004;
...
sub methods { return (stooq_api_usx2pln => \&stooq_api_usx2pln); }

{
  my @labels = qw/date isodate time method source name currency last price open high low ask bid volume/;
	
  sub labels { return (stooq_api_usx2pln => \@labels); }
}

sub stooq_api_usx2pln {
  my $quoter  = shift;
  my @symbols = @_;
  ...
  $url_rate   = $STOOQ_STOCKS_URL . '?s=USDPLN&f=c';
  ...
  foreach $symbol (@symbols) {
    ...
    unless ($date eq "N/A") {
		...
		$stocks{$symbol, 'method'}   = 'stooq_api_usx2pln';
		$stocks{$symbol, 'source'}   = 'Finance::Quote::StooqAPIusx2pln';

		$stocks{$symbol, 'currency'} = 'PLN';
	
		# divide USX (¢) quotes by 100
...
=head1 SYNOPSIS
    ...
    %stockinfo = $q->fetch("stooq_api_usx2pln","tlt"); # the letter ticker
