...
#
package Finance::Quote::StooqAPIeur;
require 5.004;
...
sub stooq_api_eur {
  ...
  foreach my $symbol (@symbols) {
    ...
    unless ($date eq "N/A") {
		...
		$stocks{$symbol, 'method'}   = 'stooq_api_eur';
		$stocks{$symbol, 'source'}   = 'Finance::Quote::StooqAPIeur';

		$stocks{$symbol, 'currency'} = 'EUR';
...
=head1 SYNOPSIS
    ...
    %stockinfo = $q->fetch("stooq_api_eur","tlt"); # the letter ticker
