...
#
package Finance::Quote::StooqAPIgbp2pln;
require 5.004;
...
sub methods { return (stooq_api_gbp2pln => \&stooq_api_gbp2pln); }

{
  my @labels = qw/date isodate time method source name currency last price open high low ask bid volume/;
	
  sub labels { return (stooq_api_gbp2pln => \@labels); }
}

sub stooq_api_gbp2pln {
  my $quoter  = shift;
  my @symbols = @_;
  ...
  foreach $symbol (@symbols) {
    ...
    unless ($date eq "N/A") {
		...
		$stocks{$symbol, 'method'}   = 'stooq_api_gbp2pln';
		$stocks{$symbol, 'source'}   = 'Finance::Quote::StooqAPIgbp2pln';
...
=head1 SYNOPSIS
    ...
    %stockinfo = $q->fetch("stooq_api_gbp2pln","tlt"); # the letter ticker
